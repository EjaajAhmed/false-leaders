...
async function commentsRoutes(server) {
    ...
    server.post('/', { onRequest: [auth_1.requireVerified] }, async (request, reply) => {
       ...
       VALUES ($1, $2, $3) RETURNING *`, [politician_id, user.id, body]);
        ...
        const { rows: otherCommenters } = await client_1.db.query(`SELECT DISTINCT user_id FROM comments WHERE politician_id = $1 AND user_id != $2`, [politician_id, user.id]);
        for (const commenter of otherCommenters) {
            await client_1.db.query(`INSERT INTO notifications (user_id, type, message, link)
         VALUES ($1, 'comment_reply', $2, $3)`, [commenter.user_id, `@${user.username} also commented on ${politicianName}`, `/politicians/${politician_id}`]);
        }
