...
export default function ThreadComposer({ board = 'general', leader = null, onDone }: Props) {
  ...
  const [brd, setBrd] = useState(BOARD_LABEL[board] ? board : 'general')
  ...
  return (
    <div className="card card--elevated stack">
      ...
      <div className="row">
        <button className="btn btn--gold" disabled={!canPost} onClick={() => create.mutate({ title: title.trim(), body: body.trim(), board: brd, politician_id: leaderPick?.id, is_anonymous: anon })}>{create.isPending ? 'Posting' : 'Post thread'}</button>
